...
class LSH(object):
    ...
    def _get_permuted_row(self, n_cols, a_coeff, b_coeff):
        ...
        for col_number in range(n_cols):
            hash_value = self.minhash(a_coeff, b_coeff, col_number, self.larger_prime)
    ...
    def _get_signature_matrix(self, permutation_matrix):
        ...
        for i in range(n_hashes):
            ...
            hashes_indexs = [(index, value) for index, value in enumerate(hashes_values)]
            lower_hashes = sorted(hashes_indexs, key=lambda x: x[1])
            
            has_signature_flags = [1 for t in range(n_rows)]   # check if each doc has a signature for the i-th hash (1:no, 0:yes)
    ...
    def _get_similarity_matrix(self, sig_matrix):
        ...
        similarity_matrix = [['INF' for i in range(n_docs)] for j in range(n_docs)]
        
        for i in range(n_docs):
            
            doc_a = set([(sig_matrix[i][s], s) for s in range(len(sig_matrix[i]))])
